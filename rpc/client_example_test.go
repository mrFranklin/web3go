...
func TestExampleClientSubscription(t *testing.T) {
	//client, _ := rpc.Dial("ws://47.92.197.201:9548")
	client, _ := rpc.Dial("ws://127.0.0.1:9548")
...
func subscribeBlocks(client *rpc.Client, subch chan <-*types.Header) {
	...
	if err := client.CallContext(ctx, &raw, "eth_getBlockByNumber", toBlockNumArg(nil), true); err != nil {
